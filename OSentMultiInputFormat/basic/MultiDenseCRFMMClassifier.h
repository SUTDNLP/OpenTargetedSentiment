...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, int wordcontext, int label1Size, int label2Size, int hiddenSize) {
    ...
    _olayer_linear1.initial(_label1Size, _hiddenSize, false, 40, 2);
    _olayer_linear2.initial(_label2Size, _hiddenSize, false, 50, 2);
    _eval2.reset();
  }
  ...
  inline dtype process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      _olayer_linear1.ComputeForwardScore(project, output1);
      ...
      _olayer_linear1.ComputeBackwardLoss(project, output1, output1Loss, projectLoss);
