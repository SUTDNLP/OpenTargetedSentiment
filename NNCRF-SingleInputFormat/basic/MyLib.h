...
const static double minlogvalue = -1000;
...
public:
  ...
  LabelScore(int id, double value) {
    labelId = id;
    score = value;
  }
};
...
inline double logsumexp(double a[], int length) {
  double max = a[0];
...
inline bool is_end_label(const string& label, const string& nextlabel) {
  ...
  if ((label[0] == 'e' || label[0] == 'E' || label[0] == 's' || label[0] == 'S') && label[1] == '-')
    return true;
  if (is_start_label(nextlabel))
    return true;
  if (nextlabel.length() == 1 && (nextlabel[0] == 'o' || nextlabel[0] == 'O'))
    return true;
