/*
 * DcombCRFMMClassifier.h
 *
 *  Created on: Mar 18, 2015
 *      Author: mszhang
 */

#ifndef SRC_DcombCRFMMClassifier_H_
#define SRC_DcombCRFMMClassifier_H_
...
class DcombCRFMMClassifier {
public:
  DcombCRFMMClassifier() {
...
public:
  ...
  inline void init(const NRMat<double>& wordEmb, int wordcontext, int label2Size, int hiddenSize, int atom_composition_layer_num,
    ...
    _olayer_denselinear2.initial(_label2Size, _hiddenSize, false, 4, 2);
    _sparselayer_linear.initial(_linearHiddenSize, _linearfeatSize, false, 4, 0);
    _olayer_sparselinear2.initial(_label2Size, _linearHiddenSize, false, 4, 2);
  ...
  inline double process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        sparseout[idx] = NewTensor<xpu>(Shape2(1, _label2Size), 0.0);
        sparseoutLoss[idx] = NewTensor<xpu>(Shape2(1, _label2Size), 0.0);
        sparse[idx] = NewTensor<xpu>(Shape2(1, _linearHiddenSize), 0.0);
        sparseLoss[idx] = NewTensor<xpu>(Shape2(1, _linearHiddenSize), 0.0);
        denseout[idx] = NewTensor<xpu>(Shape2(1, _label2Size), 0.0);
        denseoutLoss[idx] = NewTensor<xpu>(Shape2(1, _label2Size), 0.0);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        _olayer_denselinear2.ComputeForwardScore(projected[idx], denseout[idx]);
        _olayer_sparselinear2.ComputeForwardScore(sparse[idx], sparseout[idx]);
        output2[idx] = denseout[idx] + sparseout[idx];
      }
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        _olayer_sparselinear2.ComputeBackwardLoss(sparse[idx], sparseout[idx], output2Loss[idx], sparseLoss[idx]);
        _olayer_denselinear2.ComputeBackwardLoss(projected[idx], denseout[idx], output2Loss[idx], projectedLoss[idx]);
...
#endif /* SRC_DcombCRFMMClassifier_H_ */
