/*
 * MultiDenseCRFMMClassifier.h
 *
 *  Created on: Mar 18, 2015
 *      Author: mszhang
 */

#ifndef SRC_MultiDenseCRFMMClassifier_H_
#define SRC_MultiDenseCRFMMClassifier_H_
...
class MultiDenseCRFMMClassifier {
public:
  MultiDenseCRFMMClassifier() {
...
public:
  ...
  int _inputSize;
  int _inputcontext;
  UniHidderLayer<xpu> _olayer_linear1;
...
public:
  ...
  inline void init(const NRMat<double>& wordEmb, int wordcontext, int label1Size, int label2Size, int hiddenSize, int atom_composition_layer_num) {
    ...
    _olayer_linear1.initial(_label1Size, _hiddenSize, false, 4, 2);
    _olayer_linear2.initial(_label2Size, _hiddenSize, false, 5, 2);
  ...
  inline double process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      Tensor<xpu, 2, double> projectedLossTmp = NewTensor<xpu>(Shape2(1, _hiddenSize), 0.0);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        _olayer_linear1.ComputeForwardScore(projected[idx], output1[idx]);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        _olayer_linear1.ComputeBackwardLoss(projected[idx], output1[idx], output1Loss[idx], projectedLoss[idx]);
        _olayer_linear2.ComputeBackwardLoss(projected[idx], output2[idx], output2Loss[idx], projectedLossTmp);
        projectedLoss[idx] = projectedLoss[idx] + projectedLossTmp;
  ...
  void updateParams(double nnRegular, double adaAlpha, double adaEps) {
    _olayer_linear1.updateAdaGrad(nnRegular, adaAlpha, adaEps);
  ...
  void checkgrads(const vector<Example>& examples, int iter) {
    checkgrad(examples, _olayer_linear1._W, _olayer_linear1._gradW, "_olayer_linear1._W", iter);
    checkgrad(examples, _olayer_linear1._b, _olayer_linear1._gradb, "_olayer_linear1._b", iter);
...
#endif /* SRC_MultiDenseCRFMMClassifier_H_ */
